...
const Products = {
    createProduct : async (req,res)=>{
        const {nameuz, descriptionuz, categoryID, price} = req.body
        ...
        const product = new Product({
            ...
            categoryID, price,
            poster:{org,min}, images:urls,
        })
        await product.save()
        res.redirect('/api/product/uz')
    },
    getAll : async (req,res)=>{
        const result = await Product.find({['name.uz']:{$gte:0}})
            .sort({createdAt:-1})
            .limit(100)
            .populate('categoryID')
    ...
    productUpdate : async (req,res)=>{
        ...
        result.description.uz = descriptionuz
