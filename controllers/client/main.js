...
const main = {
    ...
    getCatalog :async (req,res)=>{ const user = req.session.user
        ...
        const category = await Category.find().sort({createdAt:-1})
        const product1 = await Product.find({categoryID:category[0]._id}).and({['name.uz']:{$gte:0}}).sort({createdAt:-1}).limit(8).select('-images -description ').populate('categoryID')
        const product2 = await Product.find({categoryID:category[1]._id}).and({['name.uz']:{$gte:0}}).sort({createdAt:-1}).limit(8).select('-images -description ').populate('categoryID')
        let product3 = []
        let product4 = []
        let product5 = []
        let product6 = []
        ...
        if(category[3]){
             product4 = await Product.find({categoryID:category[3]._id}).and({['name.uz']:{$gte:0}}).sort({createdAt:-1}).limit(8).select('-images -description ').populate('categoryID')
        }
        if(category[4]){
             product5 = await Product.find({categoryID:category[4]._id}).and({['name.uz']:{$gte:0}}).sort({createdAt:-1}).limit(8).select('-images -description ').populate('categoryID')
        }
        if(category[5]){
             product6 = await Product.find({categoryID:category[5]._id}).and({['name.uz']:{$gte:0}}).sort({createdAt:-1}).limit(8).select('-images -description ').populate('categoryID')
        }
        if(category[6]){
             product7 = await Product.find({categoryID:category[6]._id}).and({['name.uz']:{$gte:0}}).sort({createdAt:-1}).limit(8).select('-images -description ').populate('categoryID')
        }
        if(category[7]){
             product8 = await Product.find({categoryID:category[7]._id}).and({['name.uz']:{$gte:0}}).sort({createdAt:-1}).limit(8).select('-images -description ').populate('categoryID')
        }
        if(category[8]){
             product9 = await Product.find({categoryID:category[8]._id}).and({['name.uz']:{$gte:0}}).sort({createdAt:-1}).limit(8).select('-images -description ').populate('categoryID')
        }
        if(category[9]){
